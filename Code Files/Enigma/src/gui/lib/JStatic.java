...
public class JStatic extends JPanel
{
    ...
    public void redraw(final int[][] connection) {
        this.removeAll();
        for (int i = 0; i < 26; ++i) {
            this.gc.gridx = 0;
            this.gc.gridy = i;
            JLabel letter = new JLabel(new StringBuilder().append((char)(65 + i)).toString());
            letter.setFont(new Font(letter.getFont().getName(), 0, 11));
            this.add(letter, this.gc);
            this.gc.gridx = 2;
            letter = new JLabel(new StringBuilder().append((char)(65 + i)).toString());
            letter.setFont(new Font(letter.getFont().getName(), 0, 11));
            this.add(letter, this.gc);
            this.gc.gridx = 3;
            if (i == this.thickOutLine) {
                this.add(new JLabel("<<"), this.gc);
            }
            else if (i == this.thickInLine) {
                this.add(new JLabel(">>"), this.gc);
            }
            else {
                this.add(new JLabel("     "), this.gc);
        ...
        this.gc.gridy = 0;
        this.gc.gridx = 1;
        this.add(new DrawingLine(connection, null), this.gc);
    ...
    private class DrawingLine extends JPanel
    {
        ...
        public DrawingLine(final int[][] connection, final Rotor rotor) {
            ...
            this.width = JStatic.this.dim.width - 60;
        }
        ...
        public void paintComponent(final Graphics g) {
            for (int i = 0; i < this.connection.length; ++i) {
                ...
                if (JStatic.this.thickInLine == i || JStatic.this.thickOutLine == i) {
        ...
        public Dimension getPreferredSize() {
            ...
            return new Dimension(this.width + 1, JStatic.this.dim.height);
        }
    }
}
